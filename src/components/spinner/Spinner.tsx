...
const SpinnerDefaultProps = {
	size: 'sm',
	align: 'center',
	text: __( 'Loading…', 'innovator-ai' ),
};

const Spinner = ( { align, text }: ISpinner ) => {
	const textAlignClassName = typeof align !== 'undefined' ? `text-${ align }` : `text-${ SpinnerDefaultProps.align }`;
	const spinnerText = typeof text !== 'undefined' ? text : SpinnerDefaultProps.text;

...
namespace WpInnovator\InnovatorAi\Blocks;
...
class Manager {
	...
	public function register_all_blocks(): void {
        ...
        if ( ! innovator_ai()->settings->get()['enable_ai'] || empty( innovator_ai()->settings->get()['api_key'] ) ) {
            return;
        }
    ...
    protected function register_block_metas(): void {
        ...
        foreach ( $blocks as $block ) {
            $block_folder    = INNOVATOR_AI_PATH . '/build/blocks/' . $block;
            $block_options   = [];
            $markup_file_path = INNOVATOR_AI_TEMPLATE_PATH . '/blocks/' . $block . 'markup.php';
